...
type BIFBlockHeader struct {
	...
	AccountTreeHash    string `json:"account_tree_hash"`    //账户树哈希
	ConsensusValueHash string `json:"consensus_value_hash"` //共识信息哈希
	Hash               string `json:"hash"`                 //区块哈希
	PreviousHash       string `json:"previous_hash"`        //上一区块哈希
	FeesHash           string `json:"fees_hash"`            //费用哈希
	ValidatorsHash     string `json:"validators_hash"`      //共识列表哈希
}
...
type BIFBlockGetLatestInfoResponse struct {
	...
	LedgerLength int64                       `json:"ledger_length"` //区块头大小（bytes）
}
